    ...
    <div className={styles.profile}>
      <div className={styles.profileHeader}>
        ...
        <div className={styles.profileInfo}>
          ...
          {cv.general.websiteURL ?
            <a className={styles.website} href={cv.general.websiteURL}>{cv.general.website}</a>
          : null}
        </div>
      </div>
  ...
  return (
    <div className={styles.experience}>
      ...
      <div className={styles.experienceContent}>
        ...
        <div className={styles.description}>
          {/* split desc by \n, strip preceding "*'s", create UL, LIs */}
          {/* <RichText text={experience.description}/> */}
          <ul>
            {experience.description.split("\n").map((item: string, index: number) => {
              if (item.trim() === "") return null;
              return <li className={styles.descriptionItem} key={index}>{item.replace(/^[*-]\s*/, "")}</li>
            })}
          </ul>
        </div>
        : null}
